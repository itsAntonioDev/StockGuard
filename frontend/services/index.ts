...
export const authService = {
  ...
  confirmMfaSetup: (code: string, rememberDevice = false) => api<{ ok: true }>('/auth/mfa/confirm', { method: 'POST', body: { code, rememberDevice } }),
  verifyMfa: (code: string, rememberDevice = false) => api<{ ok: true }>('/auth/mfa/verify', { method: 'POST', body: { code, rememberDevice } }),
  forgetTrustedDevice: () => api<void>('/auth/trusted-device', { method: 'DELETE' }),
...
export const catalogService = {
  ...
  deleteLot: (lotId: string) => api<void>(`/lots/${lotId}`, { method: 'DELETE' }),
